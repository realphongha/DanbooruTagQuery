...
def predict(image_path, checkpoint, top_k=10, ignored_tags=None):
    config = TrainConfig(); state = torch.load(checkpoint, map_location="cpu", weights_only=False); config.num_classes = len(state["tag_to_id"]); model = ImageTagger(config.model_name, config.num_classes, pretrained=False, head_type=config.head_type); model.load_state_dict(state["model"]); model.eval()
    ...
    if ignored_tags: results = [(tag, score) for tag, score in results if tag not in ignored_tags]
    return results[:top_k]

if __name__ == "__main__":
    parser = argparse.ArgumentParser(); parser.add_argument("image"); parser.add_argument("checkpoint"); parser.add_argument("--top-k", type=int, default=10); parser.add_argument("--ignore-file", default="data/ignored_tags.txt"); args = parser.parse_args()
