...
def predict(image_path, checkpoint, top_k=None, min_score=0.0, ignored_tags=None):
    ...
    results = sorted(((inverse[index.item()], float(scores[index.item()])) for index in scores.argsort(descending=True)), key=lambda x: x[1], reverse=True)
    if ignored_tags:
...
if __name__ == "__main__":
    ...
    print("\n".join(f"{tag:<24} {score:.4f}" for tag, score in predict(args.image, args.checkpoint, args.top_k, args.min_score, ignored)))
